...
| Fecha      | Autor       | Cambio                                                                                                                                                                                                                                                                                                                                                                                                             |
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 2026-05-09 | Cursor      | **Deps Node alineadas local ↔ CI:** `node_modules/` raíz no versionado; CI: `npm ci` raíz + `backend` con `npm ci --omit=dev` (smoke no usa devDeps backend). §2.1.                                                                                                                                                                                            |
...
**Dependencias Node (misma secuencia que CI):** hay dos `package.json`: raíz (scripts `npm run …`, p. ej. `test:ci`) y `backend/` (servidor, `pg`, Express). Tras clonar: `npm ci` en la raíz; en `backend/`, `npm ci` completo para desarrollo local, o `npm ci --omit=dev` si solo necesitás ejecutar tests/smoke como en GitHub Actions (sin `firebase-tools` / Tailwind). **No** commitear `node_modules/`.
