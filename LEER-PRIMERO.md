...
| **CSS / UI**                                                                 | Tokens en `backend/tailwind.config.js`; capa de componentes en `frontend/public/css/source.css`; estándar operativo (layout, sidebar, responsive, botones, **tablas SPA:** tarjetas en móvil **§6.1** `spa-md-table-wrap` / `spa-md-cards-wrap`, **formato tabla en PC §6.2**, **formularios multipaso §6.3** `spa-form-page` / `spa-form-section` / `spa-form-summary-*`, build CSS): **`.cursor/rules/20-frontend-design-system.mdc`**. Botones de panel: `.btn-primary`, `.btn-outline`, `.btn-danger`, `.btn-success`, `.btn-warning`, etc.       |
| **Auditorías y build tras cambiar código**                                   | El agente **debe ejecutar** en terminal los comandos pertinentes al alcance (no solo sugerirlos): desde la raíz, `node scripts/tooling/audit-ui-monitored.js` si hubo UI/markup/CSS; `node scripts/tooling/audit-complexity-monitored.js` si hubo lógica/servicios/rutas; `npm run build:css` en `backend/` si cambió `source.css`, tokens Tailwind o clases nuevas en vistas. Si el bloque mezcla ámbitos, correr **todos** los que apliquen. Umbrales y detalle en `CLAUDE.md`; política Cursor en `.cursor/rules/40-cursor-backlog-coordinacion.mdc` §2. |
...
| **Backend / API pública (host frente a clientes)** | **`https://rezerva.cl`** — dominio custom que hoy enruta al servicio **Render** del repo (mismo backend que marketplace). La URL `*.onrender.com` del servicio puede existir para operación interna; para OpenAPI, extensiones IA y smokes HTTP usar **`https://rezerva.cl`** como base (coherente con `servers.url` en los YAML). Si el dashboard de Render muestra otra URL canónica o el custom domain cambia, **actualizar esta fila** y los `servers` de los OpenAPI en el mismo commit. |
| **OpenAPI (ChatGPT / Actions, verificación de contrato)** | `https://rezerva.cl/openapi-chatgpt.yaml` y `https://rezerva.cl/openapi-gemini.yaml` (servidos por el backend). |
| **Health de versión de contrato API** | `GET https://rezerva.cl/api/public/version` — campo `version` alineado a `info.version` del OpenAPI (p. ej. 1.4.8) salvo `PUBLIC_API_CONTRACT_VERSION` en env. |
| **GPT en ChatGPT (nombre del conector / asistente comercial)** | **SuiteManager Marketplace IA** — el Action debe apuntar a **`https://rezerva.cl`** para esquema y URLs base. Tras cambiar el OpenAPI: **reimportar el schema** en la configuración del GPT, **guardar** y probar en **un chat nuevo** (evita herramientas desactualizadas). La edición/prueba fiable de GPT con Actions suele requerir **ChatGPT Plus** (o plan equivalente); sin Plus puede quedar **pendiente** la verificación conversacional. |
...
| **Panel admin en Render (CSS/JS del SPA)** | **Start** desde la **raíz del repo**: `npm start` — ejecuta `prestart` → `npm run build` en `backend` (Tailwind → `frontend/public/css/style.css`). Si el servicio arranca solo con `node backend/index.js`, el CSS compilado puede no generarse o quedar viejo. **Build Command** recomendado incluye `npm run build` (raíz) o `cd backend && npm run build`. Variable opcional **`PANEL_ASSET_VERSION`** fuerza el query `?v=` del panel. El log `[Startup] Panel admin asset key` confirma la clave en runtime. |
...
*Última actualización: 2026-05-10 — Tabla **Auditorías y build**: el agente debe **ejecutar** la batería pertinente en terminal (no solo sugerirla); ver también `40-cursor-backlog-coordinacion.mdc` §2. Historial 2026-05-09: referencias de entorno / OpenAPI / `rezerva.cl`. Historial 2026-05-07: MCP `rezerva-mcp-server`; regla `50`; `coordinacion-cursor-paralelo.md`.*