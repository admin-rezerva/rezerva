...
| **CSS / UI**                                                                 | Tokens en `backend/tailwind.config.js`; capa de componentes en `frontend/public/css/source.css`; estándar operativo (layout, sidebar, responsive, botones, **tablas SPA → tarjetas en móvil** `spa-md-table-wrap` / `spa-md-cards-wrap`, build CSS): **`.cursor/rules/20-frontend-design-system.mdc`** (§6.1). Botones de panel: `.btn-primary`, `.btn-outline`, `.btn-danger`, `.btn-success`, `.btn-warning`, etc.       |
