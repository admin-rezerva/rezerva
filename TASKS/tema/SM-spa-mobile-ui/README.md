...
- **2026-05-11 — Lista de espera de disponibilidad (`/gestion-diaria/espera-disponibilidad`):**
  ...
  - Implementación: `frontend/src/views/gestionarEsperaDisponibilidad.js`; utilidades globales tabla/listado: `.cursor/rules/20-frontend-design-system.mdc` §6.1 y `source.css` (`.th`, `.spa-md-table-wrap` / `.spa-md-cards-wrap`). **Despliegue:** el panel se sirve desde el repo en Render; si no ves el layout nuevo, confirma que `main` incluye el commit y forzar recarga (Ctrl+F5). Rutas con guiones bajos (`/gestion_diaria/espera_disponibilidad`) se normalizan a `/gestion-diaria/espera-disponibilidad` en `router.js`.
