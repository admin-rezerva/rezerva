...
| `SM-rebrand-dominio` | Rebrand dominio (suitemanagers.com → rezerva.cl) + limpieza Firebase | En curso (Cursor) | — | `tema/SM-rebrand-dominio` | Cursor | 2026-05-08 — **Correo:** `emailService.js` (From `EMAIL_FROM`+`EMAIL_FROM_NAME`, Reply-To `EMAIL_SUPPORT`, SMTP pool `EMAIL_HOST`/`EMAIL_PORT`), copias Rezerva en jobs/servicios de mail; `.env.example` + runbook Resend/SMTP actualizados. 2026-05-08 — Marketplace móvil (`rezerva.cl`): CTA colapsable + panel expandido (Airbnb-like). 2026-05-08 — Auditoría identidad: `audit-cuentas-correo-identidad-plataforma.md`. **Pendiente:** OAuth rotado; Fase 4 PG; `verify-subdomain-logic` | `tema/SM-rebrand-dominio/runbook-migracion-resend-administrador-rezerva.md`, `tema/SM-rebrand-dominio/audit-cuentas-correo-identidad-plataforma.md`, `pendientes-y-registro.md`, `plan-accion-rebrand.md`, `README.md` |
