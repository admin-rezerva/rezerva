...
| `SM-rebrand-dominio` | Rebrand dominio (suitemanagers.com → rezerva.cl) + limpieza Firebase | En curso (Cursor) | — | `tema/SM-rebrand-dominio` | Cursor | 2026-05-10 — **OAuth Google Contacts (panel):** redirect URI resuelto por env (`PANEL_PUBLIC_ORIGIN`, `GOOGLE_OAUTH_REDIRECT_URI`, `RENDER_EXTERNAL_URL` o fallback JSON) en `authGoogle.js` / `googleContactsService.js`; `GET /api/config/platform` expone `panelPublicOrigin`; vista SPA `autorizarGoogle.js` + `.env.example`. En Google Cloud: añadir URI de redirección y dominios autorizados para `rezerva.cl` (o host real). 2026-05-08 — Doc Firebase, marketplace Airbnb-like, correo Nodemailer, etc. **Pendiente:** verificación OAuth en prod; Fase 4 PG; `verify-subdomain-logic` | `tema/SM-rebrand-dominio/runbook-migracion-resend-administrador-rezerva.md`, `backend/config/googleOAuthRedirectUri.js`, `backend/routes/authGoogle.js`, `pendientes-y-registro.md`, `README.md` |
