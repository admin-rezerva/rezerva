...
| `SM-spa-mobile-ui` | SPA responsive (gestión diaria, modales) | En curso (Cursor) | — | `tema/SM-spa-mobile-ui` | Cursor | 2026-05-11 — **Caché panel:** `index.html` inyecta `adminBootstrap.js?v=` (`RENDER_GIT_COMMIT` / mtime local); `adminBootstrap` propaga `?v` a `style.css`, `router.js` y vista lista de espera (import dinámico) para evitar JS viejo en CDN/navegador. 2026-05-11 — Lista de espera PC 6 col / móvil tarjetas; URL legacy `_`. 2026-05-10 — Registrar pago / Autorizar Google. | `backend/index.js`, `adminBootstrap.js`, `router.js`, `gestionarEsperaDisponibilidad.js`, `tema/SM-spa-mobile-ui/README.md` |
