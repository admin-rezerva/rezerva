...
| `SM-rebrand-dominio` | Rebrand dominio (suitemanagers.com → rezerva.cl) + limpieza Firebase | En curso (Cursor) | — | `tema/SM-rebrand-dominio` | Cursor | 2026-05-08 — **`main`** en **`admin-rezerva/rezerva`** (push OK); historial sin `google_credentials.json` (filter-branch + gc). **Pendiente:** rotar OAuth Google si aplica; Render reconnect; Fase 1/3 ops, Fase 4 PG, `verify-subdomain-logic` | `tema/SM-rebrand-dominio/pendientes-y-registro.md`, `plan-accion-rebrand.md`, `README.md` |
