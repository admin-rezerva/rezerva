...
| `SM-venta-ia` | Canales venta / venta por IA (OpenAPI, MCP, feeds) | En curso (Cursor) | — | `tema/SM-venta-ia` | Cursor | 2026-05-09 — **Gemini / OpenAPI:** `servers.url` en `openapi-gemini.yaml` + `openapi-chatgpt.yaml` alineado a **`https://rezerva.cl`** (el host `suite-manager.onrender.com` devolvía 404 en `/api/public/*` y schema; smokes deben usar el dominio que responde 200). `LEER-PRIMERO` § referencias API/OpenAPI actualizado; `gemini-smoke-instrucciones.md` § contrato/versión. Tras deploy: reimportar schema en Gemini/ChatGPT. | `tema/SM-venta-ia/venta-ia.md`, `tema/SM-venta-ia/qa-y-seguimiento-prelaunch-canales.md`, `tema/SM-venta-ia/qa-feed-ari-checklist.md`, `tema/SM-venta-ia/gemini-smoke-instrucciones.md`, coordinación, regla `45-canales-venta-solo-cursor.mdc` |
...
| `SM-rebrand-dominio` | Rebrand dominio (suitemanagers.com → rezerva.cl) + limpieza Firebase | En curso (Cursor) | — | `tema/SM-rebrand-dominio` | Cursor | 2026-05-08 — Doc: `README.md` § Firebase / `serviceAccountKey.json` (project_id legacy `suite-manager-app`, sin cambiar JSON para “rebrand”). 2026-05-08 — Marketplace SSR UI referencia Airbnb (benchmark): header #FFF + borde #DDD, pie #F7F7F7, texto #222/#717171, CTA búsqueda #FF385C; logo header `logo-bg-white`, footer `logo-transparent` compacto (~32px). 2026-05-08 — Marca gráfica (OG 1200×630, favicon, JSON-LD); script `optimize-marketplace-branding.js`. 2026-05-08 — **Correo:** Logs diagnóstico Render en `emailService.js` (`[EmailService]` arranque + cada `sendMail`). 2026-05-08 — **Correo:** Resend retirado del código; solo Nodemailer + Gmail SMTP (`emailService.js`, deps); env `EMAIL_USER` / `EMAIL_PASS` / `EMAIL_FROM`; debug GET `/empresa` sin `configuracion: AUSENTE` engañoso (hist.). 2026-05-08 — **Correo (cabeceras):** Reply-To plataforma; purga overrides en `empresaService`; copy en `empresa.js`; backlog §1.6. 2026-05-08 — Marketplace móvil (`rezerva.cl`): CTA colapsable + panel expandido (Airbnb-like). 2026-05-08 — Auditoría identidad: `audit-cuentas-correo-identidad-plataforma.md`. **Pendiente:** OAuth rotado; Fase 4 PG; `verify-subdomain-logic` | `tema/SM-rebrand-dominio/runbook-migracion-resend-administrador-rezerva.md`, `tema/SM-rebrand-dominio/audit-cuentas-correo-identidad-plataforma.md`, `pendientes-y-registro.md`, `plan-accion-rebrand.md`, `README.md` |
