...
| `SM-mail-events-mtx` | Correos transaccionales — flujo, plantillas, notificaciones y matriz de eventos | En curso (Cursor) | — | `tema/SM-mail-events-mtx` | Cursor | 2026-05-10 — HTML plantillas (`[[HTML_EMAIL]]`), IA layouts huésped/admin, etiquetas panel+cupón; admin reserva web → `notificacion_interna`; interna ignora interruptor correos huésped. **Pendiente:** regenerar plantillas en UI. | `plan-accion-correos-flujo-plantillas.md`, `plantillasIa.js`, `plantillasService.js`, `publicWebsiteService.js`, `transactionalEmailService.js` |
