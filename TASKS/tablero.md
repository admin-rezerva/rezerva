...
| `SM-mail-events-mtx` | Correos transaccionales — flujo, plantillas, notificaciones y matriz de eventos | En curso (Cursor) | — | `tema/SM-mail-events-mtx` | Cursor | 2026-05-13 — Plantillas fijas confirmación: bloque abono + datos transferencia (`BLOQUE_ABONO_TRANSFERENCIA_*`) bajo desglose precio; variables desde `datosBancarios` + `resolveDepositoReservaWeb`. URLs públicas: `obtenerBaseUrlPublica` prioriza `{sub}.rezerva.cl`. | `plantillasEmailTemplates.js`, `transactionalEmailService.js`, `plantillasEtiquetasCatalog.js`, `empresa.js`, `plantillasEtiquetasMotorFallback.js` |
