...
| `SM-pagespeed-lighthouse-max` | Core Web Vitals / Lighthouse máximo (SSR tenant + plataforma rezerva.cl) | En curso (Claude) | — | `tema/SM-pagespeed-lighthouse-max` | Cursor/Claude | 2026-05-08 — A11y: link "Rezerva" footer → `text-white underline` (WCAG 1.4.1); logo footer: `sizes` sin srcset removido. Pendiente: compresión hero/cards (repair script `--recompress-hero-full`) | `repair-web-images.js`, `webImagesRepair*.js`, README tema |
...
| `SM-rebrand-dominio` | Rebrand dominio (suitemanagers.com → rezerva.cl) + limpieza Firebase | En curso (Cursor) | — | `tema/SM-rebrand-dominio` | Cursor | 2026-05-08 — Marketplace SSR UI referencia Airbnb (benchmark): header #FFF + borde #DDD, pie #F7F7F7, texto #222/#717171, CTA búsqueda #FF385C; logo header `logo-bg-white`, footer `logo-transparent` compacto (~32px). 2026-05-08 — Marca gráfica (OG 1200×630, favicon, JSON-LD); script `optimize-marketplace-branding.js`. 2026-05-08 — **Correo:** `emailService.js` (Reply-To solo plataforma); `empresaService` purga `replyToOverride`/`nombreRemitenteOverride` en GET/PUT; texto en `empresa.js` (contacto vs Reply-To); backlog §1.6 actualizado. 2026-05-08 — Marketplace móvil (`rezerva.cl`): CTA colapsable + panel expandido (Airbnb-like). 2026-05-08 — Auditoría identidad: `audit-cuentas-correo-identidad-plataforma.md`. **Pendiente:** OAuth rotado; Fase 4 PG; `verify-subdomain-logic` | `tema/SM-rebrand-dominio/runbook-migracion-resend-administrador-rezerva.md`, `tema/SM-rebrand-dominio/audit-cuentas-correo-identidad-plataforma.md`, `pendientes-y-registro.md`, `plan-accion-rebrand.md`, `README.md` |
