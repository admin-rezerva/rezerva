...
| `SM-mail-events-mtx` | Correos transaccionales — flujo, plantillas, notificaciones y matriz de eventos | En curso (Cursor) | — | `tema/SM-mail-events-mtx` | Cursor | 2026-05-10 — IA confirmación: modelo 3 capas en `plantillasIa.js`; campo panel **Tarjetas centrales** (`instruccionesTarjetas`) + doc `modelo-correo-tres-capas-ia.md`. Roadmap: imagen→Gemini en `TEMPLATE_GENERATION`. | `plantillasIa.js`, `plantillas.modals.js`, `modelo-correo-tres-capas-ia.md` |
