...
| `SM-mail-events-mtx` | Correos transaccionales — flujo, plantillas, notificaciones y matriz de eventos | En curso (Cursor) | — | `tema/SM-mail-events-mtx` | Cursor | 2026-05-10 — Selección de plantilla dedicada por disparador (evita mismo HTML huésped/admin si una fila tiene ambos triggers); etiquetas `[LINK_CONFIRMACION_PUBLICA]`, `[EMPRESA_GOOGLE_MAPS_LINK]`; prompt IA confirmación cliente más cercano a maquetas. **Pendiente:** regenerar plantillas huésped/admin en panel y separar disparadores por fila. | `plantillasIa.js`, `plantillasEtiquetasCatalog.js`, `transactionalEmailService.js` |
