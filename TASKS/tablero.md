...
| `SM-spa-mobile-ui` | SPA responsive (gestión diaria, modales) | En curso (Cursor) | — | `tema/SM-spa-mobile-ui` | Cursor | 2026-05-11 — **Lista de espera:** layout PC + normalización de URL legacy `…/gestion_diaria/…` → `…/gestion-diaria/…` en `router.js`. **Prod:** el cambio solo se ve en Render tras **push a `main`** (módulos ES sin bundle). 2026-05-11 — Estándar tabla PC (avatar, estadía, detalles, pills) / móvil tarjetas. 2026-05-10 — Registrar pago / Autorizar Google. | `tema/SM-spa-mobile-ui/README.md`, `gestionarEsperaDisponibilidad.js`, `router.js` |
